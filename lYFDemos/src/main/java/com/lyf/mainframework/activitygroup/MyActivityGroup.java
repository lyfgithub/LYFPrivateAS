...
public class MyActivityGroup extends ActivityGroup{
	...
	private ImageView iv_tab_bg_current;
	...
	private void mFindViewById() {
		...
		GetDispalyDescription();
		iv_tab_bg_current = (ImageView)findViewById(R.id.iv_tab_bg_current);
	}
	...
	class MyOnPageChangeListener implements OnPageChangeListener{
		...
		public void onPageSelected(int position) {
			...
			//动画
			doAnimator(iv_tab_bg_current, currentPageIndex, position, tabWidth);
	...
	private void setSelectStyle(int position){
		//还原初始样式
		ll_tab1.setBackgroundColor(0x55FFFFFF);
		ll_tab2.setBackgroundColor(0x55FFFFFF);
		ll_tab3.setBackgroundColor(0x55FFFFFF);
		//改变指定tab的样式
		switch (position) {
		case 0:
	...
	/** 设置动画 */
	private void doAnimator(View target,int fromTab,int toTab,float tabWidth){  //(1,2)(2,3),(1,3)  //(0,1),
		float start=fromTab*tabWidth; //0  1  0  2
		float end=toTab*tabWidth;     //1  2  2  0
		
		Animation animation = null;
		animation = new TranslateAnimation(start, end, 0, 0);
		animation.setFillAfter(true);// True:图片停在动画结束位置
		animation.setDuration(500);  // 持续的时间为0.5s
		target.setAnimation(animation);
	}
	
    /** 获取当前屏幕 */
    public void GetDispalyDescription(){
        Display currDisplay = this.getWindowManager().getDefaultDisplay();//获取屏幕当前分辨率
        int displayWidth = currDisplay.getWidth();
        int displayHeight = currDisplay.getHeight();
        tabWidth=displayWidth/3;
